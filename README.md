...
核心系统只负责任务状态机、调度、Agent 状态、日志事件和插件编排；Meegle、Codex、Docker runtime 等具体能力作为默认内置插件接入。
...
5. Agent runtime plugin 提供隔离运行环境，默认实现是 Docker agent pool。
...
- `DockerAgentService` / `AgentRuntimePlugin`：创建 agent container，并为执行引擎提供命令运行环境。
