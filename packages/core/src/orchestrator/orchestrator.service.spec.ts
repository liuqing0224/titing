...
describe("OrchestratorService", () => {
  it("runs an initial poll on application bootstrap", async () => {
  ...
  it("keeps refreshing heartbeat while Codex execution is still running", async () => {
    ...
    const runner = {
      ...
      getExecutionContext: jest.fn(
        (runnerTask: Task): ExecutionContext => ({
          ...
          repoRoot: `${process.cwd()}/${runnerTask.repo}`,
          worktreePath: `/workspace/${runnerTask.repo}`,
          cloneUrl: null,
    ...
    resolveRun({
      ...
      repo: task.repo,
      branch: task.branch,
      repoRoot: `${process.cwd()}/${task.repo}`,
      worktreePath: `/workspace/${task.repo}`,
