...
const createTaskStore = (initialTask?: Task) => {
...
describe("TaskService", () => {
  it("enqueues a pending task", async () => {
    const repository = createTaskStore(createTask());
    ...
    expect(repository.saveTask).toHaveBeenCalledWith(expect.objectContaining({ status: "queued" }));
  ...
  it("claims a queued task and records runtime fields", async () => {
    const repository = createTaskStore(createTask({ status: "queued" }));
    const eventsService = createEventsService();
    const service = new TaskService(repository as never, createLogService() as never, eventsService as never);
  ...
  it("marks task failed when edited execution fields are invalid", async () => {
    const repository = createTaskStore(createTask({ status: "failed" }));
  ...
  it("generates a feature branch when enqueuing a pending task with empty branch", async () => {
    ...
    const repository = createTaskStore(createTask({ branch: "   " }));
    const service = new TaskService(repository as never, createLogService() as never, createEventsService() as never);
