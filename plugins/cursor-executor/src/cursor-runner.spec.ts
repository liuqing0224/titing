...
describe("CursorRunner", () => {
  it("appends streaming stdout and stderr logs during workflow execution", async () => {
    const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cursor-runner-workspace-"));
    const repoRoot = fs.mkdtempSync(path.join(os.tmpdir(), "cursor-runner-repo-"));
    fs.mkdirSync(path.join(repoRoot, ".git"), { recursive: true });

    const task = createTask({ repo: repoRoot });
    const agent = createAgent();
    const configService = new ConfigService({
      CURSOR_WORKDIR: workspaceRoot,
      CURSOR_CLI_BIN: "agent",
      CURSOR_TIMEOUT_MS: "5000"
    });
    const worktreeRoot = new CursorRunner(configService).getExecutionContext(task).worktreePath;
    ...
    const agentCalls = runtime.runCommand.mock.calls.filter((call) => call[1].command === "agent");
    expect(agentCalls.length).toBeGreaterThan(0);
    const agentArgs = agentCalls[0][1].args as string[];
    expect(agentArgs).toContain("-p");
    expect(agentArgs).toContain("--workspace");
    expect(agentArgs).toContain(worktreeRoot);
