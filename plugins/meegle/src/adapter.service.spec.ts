...
const createTaskStore = (initialTasks: Task[] = []) => {
  ...
  return {
    findTaskByExternalId: jest.fn(async (externalId: string) =>
      Array.from(store.values()).find((task) => task.externalId === externalId) ?? null
    ),
    getTask: jest.fn(async (id: string) => store.get(id) ?? null),
    listTasks: jest.fn(async () => Array.from(store.values())),
    createTask: jest.fn((input: Partial<Task>) => input as Task),
...
describe("AdapterService", () => {
  it("creates failed task and log when synced task lacks execution fields", async () => {
    const repository = createTaskStore();
  ...
  it("resets done task to pending when repo branch or instruction changes", async () => {
    const repository = createTaskStore([createTask({ status: "done" })]);
    const service = new AdapterService(
      repository as never,
      createLogService() as never,
      createMeegleAdapter([
  ...
  it("opens browser and continues sync when Meegle login is required", async () => {
    ...
    expect(settingsService.setMeegleLoginState).toHaveBeenCalledWith(
      expect.objectContaining({
        browserPending: true,
    ...
    expect(settingsService.setMeegleLoginState).toHaveBeenLastCalledWith({
  ...
  it("preserves an existing branch when later sync payload omits branch", async () => {
    const repository = createTaskStore([
      createTask({
        status: "failed",
        branch: "feature/20260507154202"
      })
    ]);
    const service = new AdapterService(
      repository as never,
    ...
    expect(repository.store.get("auto-existing")?.branch).toBe("feature/20260507154202");
  });
